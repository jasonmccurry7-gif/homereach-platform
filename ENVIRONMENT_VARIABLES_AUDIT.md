...
- Design/local visibility: `CANVA_CLIENT_SECRET`, `CANVA_TOKEN_ENCRYPTION_KEY`, `CANVA_ACCESS_TOKEN`, `GOOGLE_BUSINESS_PROFILE_CLIENT_SECRET`, `GOOGLE_BUSINESS_PROFILE_TOKEN_ENCRYPTION_KEY`
...
| Canva Connect | `CANVA_CLIENT_ID`, `CANVA_CLIENT_SECRET`, `CANVA_REDIRECT_URI`, `CANVA_TOKEN_ENCRYPTION_KEY`, `CANVA_ACCESS_TOKEN`, `CANVA_TEMPLATE_*` | OAuth, token storage, template registry, autofill, and export endpoints are present. Live jobs require OAuth credentials, encryption key, and template IDs. |
| Google Business Profile | `GOOGLE_BUSINESS_PROFILE_CLIENT_ID`, `GOOGLE_BUSINESS_PROFILE_CLIENT_SECRET`, `GOOGLE_BUSINESS_PROFILE_REDIRECT_URI`, `GOOGLE_BUSINESS_PROFILE_TOKEN_ENCRYPTION_KEY`, `GOOGLE_BUSINESS_PROFILE_ENABLE_PUBLISHING` | OAuth, encrypted token storage, read-only location/review sync, and admin visibility are present. Public publishing remains disabled unless explicitly enabled and approval-gated. |
