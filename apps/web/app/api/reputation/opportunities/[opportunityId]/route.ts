...
import { recordReputationAction } from "@/lib/reputation/engine";
...
export async function POST(
  ...
  try {
    const result = await recordReputationAction({
      supabase: service,
      opportunityId,
      actionType,
      actorUserId: user.id,
    ...
    console.error("[reputation/action] failed", err);
    return NextResponse.json(
      { error: err instanceof Error ? err.message : "Reputation action failed" },
      { status: 500 },
    );
  }
}
