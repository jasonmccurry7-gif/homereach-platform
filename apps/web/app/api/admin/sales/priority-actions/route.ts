    ...
    if (pricingLeadIds.length > 0) {
      ...
      for (const lead of pricingLeads ?? []) {
        ...
        actions.push({
          ...
          why_it_matters:   "Pricing was sent but there is no response yet. A timely human follow-up can protect the opportunity.",
          urgency:          "high",
          revenue_potential: "$200–$900/mo",
      ...
      for (const city of cities) {
        ...
        if (closed >= 7 && closed < 10) {
          actions.push({
            ...
            what_to_do:       `Review ${city} capacity and prioritize fit-ready leads`,
            why_it_matters:   "Capacity is tightening, so use current inventory data and human-approved follow-up before making claims.",
