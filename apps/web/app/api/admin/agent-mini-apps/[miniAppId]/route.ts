...
import { updateMiniApp } from "@/lib/agent-mini-apps/mutations";
...
export async function GET(
  ...
  if (!detail.miniApp) {
    return NextResponse.json({ ok: false, error: detail.warning ?? "Mini app not found." }, { status: 404 });
  }
...
export async function PATCH(
  ...
  try {
    ...
    const result = await updateMiniApp({
