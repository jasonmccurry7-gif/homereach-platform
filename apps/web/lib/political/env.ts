...
/**
 * Reversible kill switch for the Candidate Campaign Launch Agent UI/API.
 * Defaults on with the political dashboard so launch work is visible without
 * requiring a new production env var, but can be disabled instantly.
 */
export function isCandidateLaunchAgentEnabled(): boolean {
  return process.env.DISABLE_POLITICAL_CANDIDATE_AGENT !== "true";
}
