...
describe("generatePoliticalQuote — overrides", () => {
  it("pricePerPieceCentsOverride is capped at the political postcard ceiling", () => {
    const q = generatePoliticalQuote(
      fixture({
        householdCountOverride: 10_000,
        pricePerPieceCentsOverride: 100, // $1.00/pc, capped for political postcards
      }),
    );
    expect(q.pricePerPieceCents).toBe(MAX_POLITICAL_POSTCARD_PRICE_PER_PIECE_CENTS);
    expect(q.subtotalCents).toBe(MAX_POLITICAL_POSTCARD_PRICE_PER_PIECE_CENTS * 10_000);
  });
  ...
  it("configOverride pricing remains capped at the political postcard ceiling", () => {
    const q = generatePoliticalQuote(
      fixture({
        householdCountOverride: 10_000,
        configOverride: {
  ...
  it("region multiplier cannot exceed the political postcard ceiling", () => {
    const q = generatePoliticalQuote(
      fixture({
        householdCountOverride: 10_000,
        configOverride: {
    ...
    expect(q.pricePerPieceCents).toBe(
      Math.min(MAX_POLITICAL_POSTCARD_PRICE_PER_PIECE_CENTS, base * 2),
    );
  });
});
