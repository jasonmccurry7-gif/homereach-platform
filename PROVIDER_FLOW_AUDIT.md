...
27. Stale route-local direct Twilio/Mailgun helper code was removed from sales-event and Echo agent routes; the active send paths now point at central `sendSms()` / `sendEmail()` only.
...
2. The route applies a basic public rate limit before reading the request body.
3. The route reads the raw body, rejects bodies over 750 KB, and then parses JSON.
4. The persistence helper normalizes public map-plan inputs and requires at least one selected route or political geography before Supabase service-role work can begin.
5. Empty selections return a local-only response and do not create map sessions or plan rows.
6. Valid selections continue to the existing database-backed save path when Supabase service credentials are configured.
7. Human approval remains required before using saved political plans for outreach, creative, proposal, or campaign execution.
...
2. The route applies a basic public rate limit before service-role-backed lookup.
3. Public query text is trimmed and capped at 80 characters before service-role-backed lookup.
4. Public `state` is accepted only when it is a two-letter code, and `limit` is clamped to 1 through 20.
5. The internal repository can still return the full candidate intelligence record to trusted callers.
6. The public route strips direct campaign email and phone fields before returning autocomplete suggestions.
7. The public UI uses race, geography, election, and map-hint fields for autofill; human approval remains required before outreach or campaign execution.
...
6. The log-action and preferences proxy wrappers now run `requireAdminOrSalesAgent()` before JSON parsing or proxy forwarding, then still rely on the downstream admin route guards as a second check.
...
- Sales-event and Echo no longer carry unused route-local direct Twilio/Mailgun helper implementations that could confuse provider-routing reviews.
...
- Focused ESLint on sales-event and Echo agent routes passed with 0 warnings/errors after removing stale direct Twilio/Mailgun helper code; full `pnpm test` passed with 209 tests across 33 files; full workspace typecheck passed across 5 packages; full web lint passed with 478 existing warnings and 0 errors; placeholder-env web build generated 247 static pages; `git diff --check` passed.
...
20. Probe `/api/spots/resolve` and `/api/spots/availability` with missing or invalid parameters and expect 400 plus rate-limit metadata, without depending on successful catalog records.
21. Probe `/api/political/routes/coverage` with an invalid or missing `state` and expect a read-only response plus rate-limit metadata, without creating route plans or campaign records.
21. Add admin health checks for provider telemetry freshness, not just table readability.
22. Only then perform provider-level test-mode checks.
