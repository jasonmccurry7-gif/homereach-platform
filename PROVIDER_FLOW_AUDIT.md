...
20. Public `/api/spots/resolve` and `/api/spots/availability` service-role-backed read lookups now have first-layer in-process rate limits before catalog/availability reads.
...
- Public spot slug resolution and availability checks now apply basic in-process rate limits before service-role-backed catalog/availability reads.
