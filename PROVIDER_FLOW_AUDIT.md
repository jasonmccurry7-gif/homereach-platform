...
20. Public `/api/spots/resolve` service-role read lookups now have a first-layer in-process rate limit before catalog reads.
...
- Public spot slug resolution now applies a basic in-process rate limit before service-role catalog reads.
...
- Focused public-read and shared public rate-limit tests passed with 4 tests; focused route/helper/test ESLint passed with 0 warnings/errors; focused `@homereach/web` typecheck passed; full `pnpm test` passed with 189 tests across 26 files; full workspace typecheck passed across 5 packages; full web lint passed with 494 existing warnings and 0 errors; and placeholder-env web build generated 247 static pages after adding `/api/spots/resolve` rate limits. Preview probes are pending until deployment.
...
20. Probe `/api/spots/resolve` with missing parameters and expect 400 plus rate-limit metadata, without depending on successful catalog records.
